<!--
Website text. Edit the words under each "## key" line, then commit and push.
The deploy renders this file into docs/index.html.

Rules:
- Do not rename or delete "## key" lines. Each key has one spot on the page.
- Titles, labels and buttons: one paragraph.
- Lists (lines starting with "- "): one list item per line.
- Markdown works: **bold**, *italic*, [link text](https://example.org).
- "# Heading" lines only group the file; they do not appear on the page.
...
# How it works (scrollytelling intro)
...
## join-data-partner-hint
OHDSI studies can run across participating institutions while patient-level data remains with each data partner.
